...
class PapersSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for journal in journals:
            ...
            yield response.follow(url=link, callback=self.parse_journal, 
                meta={'journal_name': vol_num})
    ...
    def parse_journal(self, response):
        ...
        for article in articles:
            ...
            yield response.follow(url=link_article, callback=self.parse_paper, 
                meta={'article_name': title})

    def parse_paper(self, response):
        paper = response.request.meta['article_name']
        keywords = response.xpath(
            "//section[@class='item keywords']/span[@class='value']/text()").re('.*')
        ...
        abstract3 = response.xpath(
            "//section[@class='item abstract']/br/text()").re('.*')
        yield {
            'paper_name': paper,
